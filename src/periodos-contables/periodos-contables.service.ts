...
export class PeriodosContablesService {
	...
	async crearPeriodo(empresaId: number, data: any) {
		...
		if (fInicio >= fFin) {
			throw new BadRequestException('La fecha de inicio debe ser cronológicamente anterior a la fecha de fin.')
		}
		...
		if (solapamiento) {
			throw new BadRequestException(`El rango ingresado entra en conflicto cronológico con el periodo existente: ${solapamiento.nombre}`)
		}
	...
	async bloquearPeriodo(periodoId: number, empresaId: number) {
		...
		if (asientosPendientes > 0) {
			throw new BadRequestException(`Acción denegada: El periodo posee ${asientosPendientes} asientos en estado PENDIENTE. Apruébelos o elimínelos antes del cierre.`)
		}
